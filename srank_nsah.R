...
##################################
session_pair_matrix <- t(combn(names(list_of_nsah_dfs), 2))

names_for_one_df <- names(list_of_nsah_dfs[[session_pair_matrix[1,1]]])
##################################
...
col_names <- names(list_of_nsah_dfs[[session_pair_matrix[1,1]]])[-c(1)]
...
for (x in 1:nrow(session_pair_matrix)) {
  ...
  cols_to_compute <- names(list_of_nsah_dfs[[session_pair_matrix[1,1]]])[-c(1)]
