clean_t_nsah_srank <- as.data.frame(t_nsah_session_pair_matrix)
t_nsah_srank_summary <- matrix(0, ncol=7, nrow=3971)
for(x in 3:ncol(clean_t_nsah_srank)) {
	clean_t_nsah_srank[,x] <- sapply(clean_t_nsah_srank[,x], as.character)
	clean_t_nsah_srank[,x] <- sapply(clean_t_nsah_srank[,x], as.numeric)

	summary_length <- length(summary(clean_t_nsah_srank[,x]))
	contains_nas <- summary_length > 6

	if(contains_nas){
		t_nsah_srank_summary[(x-2),] <- unclass(summary(clean_t_nsah_srank[,x]))
	} else {
		summary_row <- unclass(summary(clean_t_nsah_srank[,x]))
		summary_row[7] <- 0
		names(summary_row)[7] <- "NA\'s"
		t_nsah_srank_summary[(x-2),] <- summary_row
	}
	
	
}

t_nsah_srank_summary <- cbind(cols_to_compute, t_nsah_srank_summary)
colnames(t_nsah_srank_summary) <- c("node_label", "min","q1","median","mean","q3","max", "NA\'s")
t_nsah_srank_summary_df <- as.data.frame(t_nsah_srank_summary)

rm(clean_t_nsah_srank, t_nsah_srank_summary)